...
function readTextFile(file) {
  ...
  rawFile.onreadystatechange = function () {
    if (rawFile.readyState === 4 && rawFile.status == "200") {
      ...
      const popUpKnowMoreBackground = document.querySelector(
        ".pop-up-know-more-background"
      );

      const popUpMoreCloseButton = document.querySelector(
        ".pop-up-more-close-button"
      );

      const contentMainContainer = document.querySelector(
      ...
      document.querySelector(".pop-up-know-more-title").innerHTML =
        product.title;

      document.querySelector(".pop-up-know-more-text").innerHTML =
        product.popUpDescription;

      document.querySelector(".number-sessions-result").innerHTML =
        product.numberSessionsResult;

      document.querySelector(".time-session").innerHTML = product.timeSession;

      document.querySelector(".interval-session").innerHTML =
        product.intervalSession;

      document.querySelector(".video-youtube").src =
        product.srcVideo + "?rel=0&modestbranding=1&showinfo=0";
      ...
      const indexPointerCarouselContainer = document.querySelector(
        ".index-pointer-carousel-container"
      );
