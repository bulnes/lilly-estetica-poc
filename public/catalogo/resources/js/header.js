...
for (let i = 0; i < downArrowButton.length; i += 1) {
    downArrowButton[i].addEventListener('click', (event) => {
        const displaySubLinksNavBarContainerList = event.target.parentNode.parentNode.children[1].style.display;
...
const linksNavBarContainerListItens = document.querySelectorAll('.links-nav-bar-container-list-item');

for (let i = 0; i < linksNavBarContainerListItens.length; i += 1) {
    linksNavBarContainerListItens[i].children[0].children[0].addEventListener('click', (event) => {
        const displaySubLinksNavBarContainerList = event.target.parentNode.parentNode.children[1].style.display;
        if (displaySubLinksNavBarContainerList === 'block') {
            console.log(event.target.children[1])
            event.target.parentNode.children[1].style.animationName = 'arrowButtonDown';
            event.target.parentNode.parentNode.children[1].style.display = 'none';
        } else {
            event.target.parentNode.children[1].style.animationName = 'arrowButtonUp';
            event.target.parentNode.parentNode.children[1].style.display = 'block';
        }
    });
}

const subLinksNavBar = document.getElementsByClassName('sub-links-nav-bar-container-list-item');
...
for (let i = 0; i < subLinksNavBar.length; i += 1) {
    subLinksNavBar[i].addEventListener('click', (event) => {
        ...
        window.location.href = "/catalogo/product.html"
    })
}
...
function linksStyleOnClick() {
    if (windowWidth > 800) {
        function disableLinks() {
            for (let i = 0; i < linksNavBarContainerListItens.length; i += 1) {
                linksNavBarContainerListItens[i].children[0].children[0].style.fontFamily = 'Brandon-regular';
...
function listMenuHome() {
    ...
    for (let i = 0; i < listItemsMenu.length; i += 1) {
        listItemsMenu[i].addEventListener('click', (event) => {
            ...
            for (let i = 0; i < linksNavBarContainerListItens.length; i += 1) {
                linksNavBarContainerListItens[i].children[0].children[1].style.animationName = 'arrowButtonDown';
                linksNavBarContainerListItens[i].children[1].style.display = 'none';
                if (event.target.innerText === linksNavBarContainerListItens[i].children[0].children[0].innerText) {
                    linksNavBarContainerListItens[i].children[0].children[1].style.animationName = 'arrowButtonUp';
...
if (window.innerWidth > 992) {
    ...
    const optDesk = document.querySelectorAll('.sub-links-nav-bar-container-list')

    document.documentElement.onclick = function(event) {
        if (!event.target.className.includes('item-menu') && (event.target.className !== optDesk.className)) {
